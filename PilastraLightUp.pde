class PilastraLightUp {
  ...
  PilastraLightUp (int locx, int locy, int Start, int End) {
    Location = locx;
    StartLoc = Location;
    groundcollision = locy;
    Skin = (int)random(0, 3);
    Skol = loadImage("plant" + Skin + ".png");
    EntityW = Skol.width;
    EntityH = Skol.height;
    IsLethal = false;
    TookDamage = false;
    ...
    Hitboxes();
    Sniff = minim.loadFile("plantabarreirA.wav");
    Sniff2 = minim.loadFile("plantasumindo.wav");
  }
  void Hitboxes() {
    CompStart = 0;
    CompEnd = 0;
