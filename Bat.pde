class Bat {
  ...
  int Location, groundcollision, Skin, EntityW, EntityH, Direction, Speed, MaxSpeed, StartLoc, CompStart, CompEnd, ImageTable, ImagePos, frame;
  boolean IsLethal, TookDamage, detect;
  PImage Skol;
  PVector Hit1Start, Hit1End;
  AudioPlayer Sniff;

  Bat (int locx, int locy, int Start, int End) {
    Location = locx;
    StartLoc = Location;
  ...
  void MoveEntity() {
    if (Location < -400 - EntityW - 2) {
      Location = 800 + EntityW;
    }
    if (Location > 1200 + EntityW + 2) {
      Location = 0 - EntityW;
    }
